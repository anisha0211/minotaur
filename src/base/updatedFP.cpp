...
{
  stats_ = new FeasPumpStats(); // _ ADD THIS LINE
...
// MINLP _ OA rounding
// ---------------------------------------------------------------------------
bool FeasibilityPump::oaRounding_(RelaxationPtr rel)
{
  LinearFunctionPtr objLF(new LinearFunction());
  // L1 slack variables
  ...
  for (size_t i = 0; i < intVars_.size(); ++i) {
    VariablePtr v = intVars_[i];
    double target = roundedSol_[v->getIndex()];
    VariablePtr t = p_->newVariable(0.0, INFINITY, Continuous);
    slacks.push_back(t);
    objLF->addTerm(t, 1.0);
    // t >= x - target
    ...
    lf1->addTerm(v, 1.0);
    lf1->addTerm(t, -1.0);
    p_->newConstraint(FunctionPtr(new Function(lf1)), -INFINITY, target);
    // t >= -(x - target)
    LinearFunctionPtr lf2(new LinearFunction());
    lf2->addTerm(v, -1.0);
    lf2->addTerm(t, -1.0);
    p_->newConstraint(FunctionPtr(new Function(lf2)), -INFINITY, -target);
  }
  p_->changeObj(FunctionPtr(new Function(objLF)), 0.0);
  e2_->clear();
  e2_->load(p_);
  EngineStatus st = e2_->solve();
  ++stats_->numNLPs;
  // clean up slacks and constraints
  for (VariablePtr t : slacks) p_->markDelete(t);
  p_->delMarkedVars();
  p_->prepareForSolve();
  if (st != ProvenOptimal && st != ProvenLocalOptimal) {
    return false;
...
bool FeasibilityPump::shouldFP_() const
{
  ...
  bool hasInt = (sz->ints > 0 || sz->bins > 0);
  bool isMILP = (sz->nonlinCons == 0 && hasInt);
  bool isMINLP = (sz->nonlinCons > 0 && hasInt);
  return isMILP || isMINLP;
}
// ---------------------------------------------------------------------------
// main solve()
// ---------------------------------------------------------------------------
void FeasibilityPump::solve(NodePtr, RelaxationPtr rel, SolutionPoolPtr s_pool)
{
if(!rel){  
 OptionDBPtr options = env_->getOptions();
  ...
  // ---------------------------------------------------------------------------
  if (isMILP) {
    logger_->msgStream(LogInfo) << me_ << "Solving MILP directly with CPLEX\n";
    e2_->clear();
    e2_->load(p_);
    EngineStatus st = e2_->solve();
    if (st == ProvenOptimal || st == ProvenLocalOptimal || e2_->getSolution() != nullptr) {
      ConstSolutionPtr sol = e2_->getSolution();
      convertSol_(s_pool, sol);
    } else {
      logger_->msgStream(LogInfo) << me_ << "No feasible solution found for MILP\n";
    }
  }
  // -----------------------------------------------------------------------
  // MINLP path
  // ---------------------------------------------------------------------------
    ...
    while (iter < maxIter && timer_->query() < 120.0) {
      ...
      UInt h = hash_();
      if (cycle_(h)) {
        perturb_(h, intVars_.size() / 10); // Flip 10% to avoid cycling
      }
      if (!oaRounding_(rel)) {
